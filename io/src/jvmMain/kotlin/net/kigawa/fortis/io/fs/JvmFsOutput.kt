...
data class JvmFsOutput(
    ...
    override suspend fun writeAt(
        ...
        channel.write(ByteBuffer.wrap(data, dataOffset, length), offset)
    }

    override suspend fun writeAppend(
        data: ByteArray, dataOffset: Int, length: Int,
    ): Int = withContext(Dispatchers.IO) {
        writeAt(channel.size(), data, dataOffset, length)
    }
